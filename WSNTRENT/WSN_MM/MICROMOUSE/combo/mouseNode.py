...
class mouse:
    ...
    def generate_sendlist(self):
        ...
        nodeListType = [0 for x in range(size)]
        nodeListOption = [0 for x in range(size)]
        count = 0
        #x(up/down)
        for i in range(0,33):
            #y(left,right)
            for j in range(0,33):
#                nodeList[count]=self.my_map.getNode(i,j).types
                nodeListType[count] = self.my_map.getNode(i,j).types
                nodeListOption[count] = self.my_map.getNode(i,j).options
                count += 1
        nodeList = [nodeListType, nodeListOption]
        return nodeList

    #takes incoming shared mouse map and combines it with my_map
    def combine_map(self,nodeList):
        nodeListType = nodeList[0]
        nodeListOption = nodeList[1]
        count = 0
        #x(up/down)
        for i in range(0,33):
            #y(left,right)
            for j in range(0,33):
                nodeType = nodeListType[count]
                nodeOption = nodeListOption[count]
                if self.my_map.getNode(i,j).types == 8:
                    self.my_map.getNode(i,j).types = nodeType
                    self.my_map.getNode(i,j).options = nodeOption
                if self.my_map.getNode(i,j).options < nodeOption:
                    self.my_map.getNode(i,j).options = nodeOption
                #node = nodeList[count]
                #if self.my_map.getNode(i,j).types == 8:
